...
# Initialize your AI components (from your existing app.py)
def load_documents():
    """Load only the p1.pdf document"""
    all_documents = []
    
    # Load only p1.pdf file
    pdf_file = "p1.pdf"
    ...
    # Debug: Print document content lengths
...
def initialize_ai_system():
    ...
    # If no chunks created, try with simpler splitter
    if not texts:
        print("Trying with CharacterTextSplitter...")
        text_splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=0)
        texts = text_splitter.split_documents(non_empty_documents)
        print(f"Text chunks with CharacterTextSplitter: {len(texts)}")
    ...
    # Model configuration - adjusted for better responses
    ...
    prompt_template = """Based on the following context, please provide a clear and helpful answer to the question. If the information is not available in the context, say "I don't have information about that in the available documents."
...
async def chat(question: str = Form(...)):
    ...
    try:
        ...
        # Use the correct invoke method for RetrievalQA
        ...
        # Clean up the answer - remove unwanted characters and formatting
        if answer:
            # Remove repeated closing brackets and clean formatting issues
            answer = answer.replace("]]]]]]]", "").replace("]]]]", "").replace("]]]", "").replace("]]", "")
            answer = answer.replace("not applicable.", "I don't have specific information about that in the available documents.")
            answer = answer.strip()
        
        # Clean up the answer
        if not answer or answer.strip() == "" or "not applicable" in answer.lower():
            answer = "I couldn't find relevant information about that topic in our documents. Could you please rephrase your question or ask about something more specific?"
...
async def health_check():
    ...
    return {"status": "healthy", "message": "AI Chatbot is running"}
